...
def test_str_count():
    actual = sp.str_count(['aa', 'bb', ' cc', None], pattern=r'\w').to_pylist()
    expect = [2, 2, 2, None]
    assert actual == expect
    actual = sp.str_count([None, 'a', 'b'], pattern=r'a').to_pylist()
    expect = [None, 1, 0]
    assert actual == expect


def test_str_remove_ascent():
    actual = sp.str_remove_ascent(
        ['tôi thấy mệt mỏi', 'hôm nay trời nắng', None]).to_pylist()
    expect = ['toi thay met moi', 'hom nay troi nang', None]
    assert actual == expect


def test_str_replace():
    actual = sp.str_replace(
        pa.array(['aa', 'bb', 'cc']), pattern=r'a', replace='b').to_pylist()
    expect = ['ba', 'bb', 'cc']
    assert actual == expect


def test_str_replace_all():
    actual = sp.str_replace_all(
        ['aa', 'bb', 'cc', None], pattern=r'a', replace='b').to_pylist()
...
def test_str_trim():
    ...
    actual = sp.str_trim(['  aa ', ' bb  ', '  cc  '],
                         side='right').to_pylist()
    expect = ['  aa', ' bb', '  cc']
    assert actual == expect


def test_str_detect():
    actual = sp.str_detect(['aa', 'bb', 'cc', None], pattern=r'a').to_pylist()
...
def test_str_extract():
    actual = sp.str_extract(["apples x4", "bag of flour",
                            "bag of sugar", "nevermind2"], pattern=r'\d').to_pylist()
    expect = ['4', None, None, "2"]
    assert actual == expect

    actual = sp.str_extract(["apples x4x", "bag of flour", "bag of sugar",
                            "nevermind2"], pattern=r'\d(.)', group=1).to_pylist()
...
def test_str_extract_all():
    ...
    actual = sp.str_extract_all(
        ["apples x4 t6", "bag 3of flour",  "1", "ads", None], pattern=r'\d').to_pylist()
    actual
    expect = [['4', '6'], ['3'], ['1'], [], [None]]
    assert actual == expect

    actual = sp.str_extract_all(
        [None, None, None, '123'], pattern=r'\d').to_pylist()
    actual
    expect = [[None], [None], [None], ['1', '2', '3']]


def test_str_split():
    actual = sp.str_split([None, 'a,b,c', 'de,f', None],
                          pattern=r',').to_pylist()
...
def test_str_subset():
    actual = sp.str_subset(
        [None, 'Apple', 'Banana', 'Acetol'], pattern=r'^A').to_pylist()
    expect = ['Apple', 'Acetol']
    assert actual == expect
...
def test_str_pad():
    actual = sp.str_pad(['a', 'b', 'c', None], width=3,
                        side='left', pad='0').to_pylist()
    expect = ['00a', '00b', '00c', None]
    assert actual == expect
    actual = sp.str_pad(['abc def', 'this is wrong'],
                        width=4, side='right', pad='0').to_pylist()
    expect = ['abc def', 'this is wrong']


def test_str_match():
    actual = sp.str_match(['one two three', None],
                          pattern=r'.+(two).+').to_pylist()
    expect = [['two'], [None]]
    assert actual == expect
