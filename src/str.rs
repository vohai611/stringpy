...
mod tests {
    ...
    //
    //impl PartialEq for Vec<Option<&str>> {
    //   fn eq(&self, othe: &Self) -> bool {}
    //}

    macro_rules! py_arrow_array {
    ...
    #[test]
    fn it_works() {
        let test_arr = arrow2::array::Utf8Array::<i32>::from([
            Some("abc".to_string()),
            Some("def".to_string()),
            None,
        ]);
    ...
    #[test]
    fn test_str_count() {
        py_arrow_array!(vec![Some(0), None, Some(1)] => vec![Some("a"), None, Some("a1")] => str_count, vec!["\\d"]);
    }
}
